let myChart,
	transactions = [];
fetch('/api/transaction').then((a) => a.json()).then((a) => {
...
function populateChart() {
	...
	myChart = new Chart(e, {
		type: 'line',
		data: { labels: c, datasets: [ { label: 'Total Over Time', fill: !0, backgroundColor: '#6666ff', data: d } ] }
	});
}
function sendTransaction(a) {
	...
	if ('' === b.value || '' === c.value) return void (d.textContent = 'Missing Information');
	d.textContent = '';
	...
	a || (e.value *= -1),
		...
		fetch('/api/transaction', {
			...
			.then((a) => a.json())
			.then((a) => {
				a.errors ? (d.textContent = 'Missing Information') : ((b.value = ''), (c.value = ''));
			})
			.catch(() => {
